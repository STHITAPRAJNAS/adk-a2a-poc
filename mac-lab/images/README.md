...
On an Apple Silicon Mac everything is `linux/arm64` — the kind nodes, the base
image and this build — so the default is right and nothing is emulated. The flag
exists for the other cases: an Intel Mac, or building an `amd64` image for an
`amd64` EKS node group.

```bash
./build.sh --platform amd64        # Intel Mac, or amd64 EKS
